...
Dir.chdir(HERE) do
  if File.exist?("lib")
    ...
    Dir.chdir(BUNDLE_PATH) do
      puts(cmd = "env CFLAGS='#{$CFLAGS}' LDFLAGS='#{$LDFLAGS}' ./configure --prefix=#{HERE} --disable-audio-hash --disable-video-hash --disable-shared --with-pic --disable-pthread 2>&1")
      raise "'#{cmd}' failed" unless system(cmd)
  ...
  $LIBS = " -lpthread -lpHash_gem -lstdc++ -lpng -ljpeg"
end
